...
**Nom i cognoms:** David Madrid Saban
...
- [X] C) La memòria de l'app creix però la càmera segueix funcionant correctament sense cap efecte visible
- [ ] D) El recurs de càmera queda reservat pel sistema operatiu i no el pot usar cap altra app fins que es reinicia el procés o el SO el recupera
...
- [X] B) Per optimitzar el rendiment: Flutter executa el codi de serveis en un Isolate separat automàticament si és en un fitxer diferent
- [ ] C) Per evitar el límit de 500 línies per arxiu que imposa la guia d'estil de Flutter
- [ ] D) Perquè permet canviar la implementació de la font de dades (API real, mock, base de dades) sense modificar la vista, i facilita els tests unitaris aïllant la lògica de xarxa
...
- [ ] B) Usar un paquet de tercers com `gal` o `image_gallery_saver` que crida les APIs natives, i declarar el permís `WRITE_EXTERNAL_STORAGE` (Android) o `NSPhotoLibraryAddUsageDescription` (iOS)
- [X] C) Flutter té el mètode integrat `MediaStore.saveImage(xfile)` disponible des del paquet `dart:io`
...
- [ ] A) Qualsevol persona amb accés al repositori pot extreure i usar la clau, generant costos en el compte de l'owner o accedint a dades sensibles de l'API
...
- [X] D) Només és un risc si el repositori és públic; en repositoris privats no hi ha risc
